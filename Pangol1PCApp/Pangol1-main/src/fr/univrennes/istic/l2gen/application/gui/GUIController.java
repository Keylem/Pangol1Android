...
public final class GUIController extends CoreController {
    ...
    public void onLanguageChange(Locale locale) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            newView.getTablePanel().refresh();

            newView.getTablePanel().getTableView().close();
            if (currentTable != null) {
                newView.getTablePanel().getTableView().open(currentTable);
            }

            newView.getReportPanel().refresh();
        });
    }
}